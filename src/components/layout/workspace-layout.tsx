...
export function WorkspaceLayout() {
  ...
  const dataset = model.data?.training_dataset_version ?? "Memuat info dataset";
  return <div className="min-h-screen bg-[#f4f2ee] text-ink">
    ...
    <div className="lg:pl-64"><header className="sticky top-0 z-10 flex items-center justify-between gap-4 border-b border-line bg-white/95 px-5 py-4 backdrop-blur lg:px-10"><div><p className="text-xs font-bold uppercase tracking-[0.18em] text-orange-600">Ruang kerja</p><p className="mt-1 text-sm font-bold text-slate-600">{user ? roleLabel(user.role) : ""}</p></div><div className="flex items-center gap-4"><span className="hidden items-center gap-1.5 text-xs font-semibold text-slate-500 md:inline-flex"><Database size={14} />{codeLabel(dataset)}</span><div className="flex items-center gap-2"><ShieldCheck size={18} className="text-teal-600" /><span className="hidden text-xs font-bold text-slate-500 sm:inline">Sesi aman</span></div></div></header>
